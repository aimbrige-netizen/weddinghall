...
| 백업 | **비교함 링크 복사** — 비교함 전체를 주소에 담아 기기·브라우저를 넘어 복원 |
...
**나머지 지표**
```
1인당 실질단가 = 총 예상비용 ÷ (청구인원 | 예상 참석인원)   ← 사용자가 선택, 기본값 청구인원
예상 축의금    = 1인 평균 × 예상 참석 성인
순수 부담액    = 총 예상비용 − 예상 축의금
```

1인당 단가의 분모는 입력 1번 섹션에서 고릅니다. **청구인원**은 실제로 결제하는 인원이라 홀끼리 비교할 때 기준이 흔들리지 않고, **예상 참석**은 하객 한 명 대접에 든 돈을 보여줍니다. 두 값은 1번 섹션 요약에 항상 나란히 표시되고, 결과 화면에서도 고르지 않은 쪽 값을 함께 보여줍니다. 예상 참석 기준을 골랐는데 참석 인원을 넣지 않으면 청구인원으로 자동 대체합니다.
...
- **1인당 실질단가의 분모** → 기획서 1-3의 예상 참석 대신 **청구인원**으로 확정 (2026-08-13).
  입력한 청구 기준 그대로 나눠 직관적이고, 예상 참석을 입력하지 않아도 항상 계산됩니다.
...
서버가 없습니다. 입력값은 브라우저 `localStorage`에 JSON 3개(`draft` / `compare` / `step`)로 들어가고, 비교함을 꽉 채워도 **2.3KB** 수준입니다. 코드에 `fetch` · `XMLHttpRequest` · `sendBeacon` · `WebSocket`이 한 건도 없어 입력한 금액이 기기 밖으로 나가지 않습니다.

대신 영구 저장은 아닙니다. 아래 경우에 사라집니다.

| 상황 | 결과 |
|---|---|
| iOS 사파리에서 7일간 재방문 없음 | 자동 삭제 (ITP) |
| 캐시·방문기록 삭제 / 시크릿 모드 | 소실 |
| 다른 기기·다른 브라우저로 접속 | 안 보임 |
| **접속 도메인이 바뀜** | 안 보임 (저장소는 origin 단위) |

그래서 비교함 화면에 **링크 복사**를 뒀습니다. 비교함 전체가 주소 해시에 실리므로(3개 기준 약 830자) 사용자가 그 링크를 자기 메신저나 북마크에 두면 위 네 경우를 전부 우회합니다. 링크로 접속하면 비교함이 복원되고 주소는 즉시 정리되며, 기존 비교함을 덮어썼다면 토스트의 **되돌리기**로 복구할 수 있습니다.

`js/config.js`의 `shareFieldOrder`는 링크 호환성을 좌우합니다. **순서를 바꾸거나 중간에 끼워 넣으면 기존 링크가 깨지므로**, 필드를 늘릴 때는 배열 끝에만 추가하고 `SHARE_VERSION`을 올리세요.

## 방문 분석

`index.html`에 Vercel Web Analytics 스크립트가 들어 있습니다. 쿠키를 쓰지 않고 방문·이탈만 집계하며 입력한 금액은 전송하지 않습니다. **Vercel 대시보드 → 프로젝트 → Analytics → Enable** 을 눌러야 동작하고, 그 외 환경(GitHub Pages·로컬)에서는 스크립트가 404로 조용히 무시됩니다. 쓰지 않으려면 해당 `<script>` 한 줄을 지우면 됩니다.
