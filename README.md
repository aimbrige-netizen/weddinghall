...
1인당 단가의 분모는 입력 1번 섹션에서 고릅니다. **청구인원**은 실제로 결제하는 인원이라 홀끼리 비교할 때 기준이 흔들리지 않고, **예상 참석**은 하객 한 명 대접에 든 돈을 보여줍니다. 두 값은 1번 섹션 요약에 항상 나란히 표시되고, 결과 화면에서도 고르지 않은 쪽 값을 함께 보여줍니다.

**고른 기준을 앱이 임의로 바꾸지 않습니다.** 나눌 인원이 비어 있으면 다른 기준으로 대신 계산하지 않고 `—`를 표시한 뒤 입력을 요청합니다.

### 앱이 사용자 대신 정하지 않는 값

**예상 참석인원(신랑측·신부측·아동)에는 기본값을 넣지 않습니다.** 앱이 채워두면 사용자가 자기가 넣지도 않은 인원으로 계산된 결과를 보게 되기 때문입니다. 금액 칸에는 예시값이 들어가지만 참석인원은 항상 빈 칸이고, 회색 플레이스홀더로 입력 예시만 보여줍니다.

"새 홀 입력하기"로 넘어올 때 이어받는 값(참석인원·축의금 등)은 **점선 테두리와 "이전 홀" 라벨**로 표시되고, 상단에 몇 개를 가져왔는지 안내합니다. 해당 칸을 고치면 표시가 풀리고, **가져온 값 비우기**로 한 번에 지울 수 있습니다(되돌리기 지원).
